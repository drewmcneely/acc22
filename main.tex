...
\author{Drew McNeely$^{1}$ and Efstathios Bakolas$^{1}$% <-this % stops a space
\thanks{$^{1}$Drew McNeely is with Department of Aerospace Engineering, 
        The University of Texas at Austin, 7500 AE Enschede, The Netherlands
        {\tt\small drew.mcneely@utexas.edu}}%
...
The foundations of this work stem from the research of FRITZ CITATIONS, who in turn builds on the work of CHO AND JACOBS and others.

HERE IS WHAT CHO AND JACOBS DID.

HERE IS WHAT FRITZ DID.

OLD PAPERS THAT THOUGHT CATEGORY THEORY WAS A GOOD IDEA for controls..

HERE IS SOME KALMAN FILTER-RELATED PAPERS. THIS IS WHERE YOU WILL CITE SOME OF THE COOL PAPERS  LIKE RIEMANNIAN GEOMETRY STUFF.
...
The axiomatic behavioral requirements of Markov kernels are as follows:
Every space must have an identity markov kernel.
Markov kernels with compatible type signatures must compose.
A family of spaces must include the singleton.
For every two spaces, there must be a product (joint) space.
The act of joining spaces must be symmetric up to isomorphism.
Kernels must be able to be conjoined independently in parallel between their respective spaces.
There must be a unique transition going from every space to the singleton.
There must be a unique deterministic transition that acts as a duplicator.
Every transition into a joint space must be conditionable on a component of its codomain.
